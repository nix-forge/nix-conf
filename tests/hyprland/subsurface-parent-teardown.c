...
static void image_failed(
    void *data, struct wp_image_description_v1 *description, uint32_t cause, const char *message) {
  (void)data;
  (void)description;
  (void)cause;
  (void)message;
...
static struct wl_buffer *buffer(int size) {
  ...
  const int pool_size = size * size * 4;
  const size_t byte_size = (size_t)pool_size;
  int fd = memfd_create("hyprland-regression", 0);
  assert(fd >= 0);
  assert(ftruncate(fd, (off_t)byte_size) == 0);
  void *data = mmap(NULL, byte_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  assert(data != MAP_FAILED);
  memset(data, 0xff, byte_size);
  struct wl_shm_pool *pool = wl_shm_create_pool(shm, fd, pool_size);
  struct wl_buffer *b =
      wl_shm_pool_create_buffer(pool, 0, size, size, size * 4, WL_SHM_FORMAT_XRGB8888);
...
int main(int argc, char **argv) {
  ...
  struct wl_subsurface *p = wl_subcompositor_get_subsurface(subcompositor, parent, root);
  wl_subsurface_set_desync(p);
  map(parent, 100);
  struct wl_surface *child = wl_compositor_create_surface(compositor);
  struct wl_subsurface *c = wl_subcompositor_get_subsurface(subcompositor, child, parent);

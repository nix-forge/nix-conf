{ inputs, ... }: {
  perSystem =
    ...
    let
      ...
      # `local-control-secure-files` intentionally supports only Darwin: it
      # protects macOS-local state and links against the platform's system
      # libraries. Keep the checks that embed it out of Linux evaluation, not
      # merely out of execution. `nix flake check --no-build` still evaluates
      # interpolations in a check derivation, so a per-check no-op is required.
      mkDarwinLocalControlCheck =
        name: check:
        if pkgs.stdenv.hostPlatform.isDarwin then
          check
        else
          pkgs.runCommand "${name}-not-applicable" { } ''
            touch "$out"
          '';
    in
    {
      ...
      checks.local-control-database-validation =
        mkDarwinLocalControlCheck "local-control-database-validation"
          (
      ...
      checks.local-control-private-path-validation =
        mkDarwinLocalControlCheck "local-control-private-path-validation"
          (
      ...
      checks.local-control-environment-validation =
        mkDarwinLocalControlCheck "local-control-environment-validation"
          (
      ...
      checks.local-control-proxy-environment =
        mkDarwinLocalControlCheck "local-control-proxy-environment"
          (
      ...
      checks.local-control-preparation-proof-validation =
        mkDarwinLocalControlCheck "local-control-preparation-proof-validation"
          (
      ...
      checks.local-control-preparation-gate-validation =
        mkDarwinLocalControlCheck "local-control-preparation-gate-validation"
          (

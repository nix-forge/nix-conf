...
int main(int argc, char **argv) {
  require(argc == 3, "expected database path and pending/completed/denied mode");
  ...
  require(
      database->PrepareNewCrashReport(&report) == Database::kNoError, "prepare synthetic report");
  const char fixture[] = "synthetic crash-report fixture";
  require(report->Writer()->Write(fixture, sizeof(fixture)), "write fixture");
  crashpad::UUID uuid;
  require(database->FinishedWritingCrashReport(std::move(report), &uuid) == Database::kNoError,
      "finish synthetic report");
  if (mode != "pending") {
    require(database->SkipReportUpload(uuid,
                crashpad::Metrics::CrashSkippedReason::kUploadsDisabled) == Database::kNoError,
        "mark report completed without uploading");
  }
  const auto directory = path.Append(mode == "pending" ? "pending" : "completed");
  const auto lock = directory.Append(uuid.ToString() + ".lock");
  int fd = -1;
  if (mode == "denied") {
    require(geteuid() != 0, "permission test must run unprivileged");
    require(chmod(directory.value().c_str(), 0500) == 0, "make directory read-only");
  } else {
    fd = open(lock.value().c_str(), O_WRONLY | O_CREAT | O_EXCL | O_CLOEXEC, 0600);
  ...
  if (child == 0) {
    ...
    _exit(contender && contender->LookUpCrashReport(uuid, &found) == Database::kBusyError ? 0 : 1);
  }
  int status = 0;
  ...
  if (mode == "denied") {
    ...
    require(access(lock.value().c_str(), F_OK) == 0, "contender preserved held lock");
    require(close(fd) == 0 && unlink(lock.value().c_str()) == 0, "release lock");
  }
  Database::Report found;

...
Set `offline_mode = false` so media controls can download remote album artwork, including Spotify covers. Keep `external_ip_enabled = false` and `telemetry_enabled = false`, and disable unwanted online services individually. Offline mode blocks all Noctalia HTTP requests, including album art. This recommendation was corrected on 2026-09-06 after reproducing missing Spotify covers with the previous `true` setting. [Shell privacy settings](https://docs.noctalia.dev/noctalia/configuration/shell/) [Album-art investigation](noctalia-spotify-album-art-research.md)

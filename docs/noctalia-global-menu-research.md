...
- [The baseline configuration](../modules/home/desktop/noctalia.nix) reserves a fixed 34 logical-pixel top bar. Its start lane is `launcher`, `workspaces`, and `active_window`; the end lane is already dense. A prototype should replace or extend `active_window`, not append an unlimited menu strip.
